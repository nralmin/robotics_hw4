% dist_to_obst = distdn( angle, position )
...
%  obstacles in sensor range, a large value (global variable infinity)
%  is returned as a substitute for infinity.
%
%  Inputs:  angle   : Angle for the sensor reading, relative to the x
%                     axis
%           position: Robot position (row vector [x y])
%
%  Outputs: dist_to_obst : Distance to the closest obstacle in the
%                          given direction

function dist_to_obst = distfn( angle, position )
